...
async function checkInteraction() {
    ...
    utils.showLoading('Verifica delle interazioni farmacologiche...');
    ...
    try {
            ...
            resultDiv.innerHTML = `
                <div class="result-card result-warning scroll-scale">
                    <div class="result-header">
                        ...
                        <h3 class="result-title">Attenzione richiesta</h3>
                    </div>
        ...
        resultDiv.innerHTML = `
            <div class="result-card result-warning scroll-scale">
                <div class="result-header">
                    <span class="result-icon">❌</span>
                    <h3 class="result-title">Si è verificato un errore</h3>
                </div>
                <div class="result-content scroll-fade scroll-delay-1">
                    <p>Si è verificato un errore durante la verifica delle informazioni sul farmaco.</p>
...
async function testAllAPIs() {
    ...
    utils.showLoading('Test delle connessioni API...');
...
function checkAPIKeyStatus() {
    ...
    if (availableProviders.length === 0) {
        ...
        setTimeout(() => {
            utils.showAlert(`L'analisi AI è pronta!`, 'success');
        }, 1000);
    }
}
...
function enableHardwareAcceleration() {
    // Seleziona elementi che necessitano di accelerazione GPU
    ...
    acceleratedElements.forEach(selector => {
        ...
        elements.forEach(el => {
            // Applicare transform3d per accelerazione GPU
    ...
    // Aggiungere will-change agli elementi animati
    const animatedElements = document.querySelectorAll('.scroll-fade, .scroll-hidden, .scroll-scale');
    animatedElements.forEach(el => {
        el.style.willChange = 'opacity, transform';
    });
    
    console.log('⚡ Accelerazione hardware abilitata');
}

// Initialize event listeners
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Inizializzazione pagina italiana avviata');
...
function showPrivacyPolicy() {
    const modal = createInfoModal('Politica sulla Privacy', `
        <div style="line-height: 1.6; color: var(--text-secondary);">
            <h4 style="color: var(--text); margin-bottom: 1rem;">🔒 Politica sulla Privacy</h4>
            
            <h5 style="color: var(--primary); margin: 1.5rem 0 0.5rem;">Raccolta Dati</h5>
            <p>Questo servizio raccoglie le seguenti informazioni:</p>
            <ul style="margin-left: 1rem;">
                <li>Nomi di farmaci cercati (memorizzati solo in archiviazione locale)</li>
                <li>Chiavi API per analisi IA (memorizzate solo in archiviazione locale)</li>
                <li>Statistiche di utilizzo (anonimizzate)</li>
            </ul>
            
            <h5 style="color: var(--primary); margin: 1.5rem 0 0.5rem;">Sicurezza Dati</h5>
            <p>Tutti i dati sensibili sono memorizzati solo nell'archiviazione locale del browser e non vengono trasmessi a server esterni. Le chiavi API vengono crittografate prima della memorizzazione.</p>
            
            <h5 style="color: var(--primary); margin: 1.5rem 0 0.5rem;">Uso dei Cookie</h5>
            <p>Questo servizio utilizza solo l'archiviazione locale per funzionalità essenziali e non utilizza cookie di tracciamento.</p>
            
            <p style="margin-top: 1.5rem; padding: 1rem; background: var(--surface); border-radius: 8px; border: 1px solid var(--border);">
                <strong>Importante:</strong> Questo strumento è solo a scopo educativo e non sostituisce il consiglio medico professionale.
            </p>
        </div>
    `);
}
...
function showTermsOfService() {
    const modal = createInfoModal('Termini di Servizio', `
        <div style="line-height: 1.6; color: var(--text-secondary);">
            <h4 style="color: var(--text); margin-bottom: 1rem;">📋 Termini di Servizio</h4>
            
            <h5 style="color: var(--primary); margin: 1.5rem 0 0.5rem;">Scopo del Servizio</h5>
            <p>Il Verificatore di Interazioni Farmacologiche è destinato solo a scopi educativi e informativi. Non è destinato a consigli medici, diagnosi o trattamento.</p>
            
            <h5 style="color: var(--primary); margin: 1.5rem 0 0.5rem;">Responsabilità dell'Utente</h5>
            <ul style="margin-left: 1rem;">
                <li>Consultare sempre professionisti sanitari prima di assumere farmaci</li>
                <li>Non fare affidamento esclusivamente sui risultati dello strumento</li>
                <li>Contattare professionisti sanitari immediatamente in situazioni di emergenza</li>
                <li>Gestire in sicurezza le chiavi API</li>
            </ul>
            
            <h5 style="color: var(--primary); margin: 1.5rem 0 0.5rem;">Limitazione di Responsabilità</h5>
            <p>Questo servizio è fornito solo a scopo informativo e non si assume responsabilità per problemi derivanti da interazioni farmacologiche.</p>
            
            <h5 style="color: var(--primary); margin: 1.5rem 0 0.5rem;">Modifiche del Servizio</h5>
            <p>Il contenuto del servizio può essere modificato senza preavviso. Le funzionalità possono essere aggiunte o modificate per miglioramenti continui.</p>
            
            <div style="margin-top: 1.5rem; padding: 1rem; background: var(--surface); border-radius: 8px; border: 1px solid var(--border);">
                <strong>⚠️ Clausola di Esclusione Medica:</strong><br>
                Questo strumento non può sostituire il consiglio medico professionale. Consultare professionisti sanitari prima di prendere decisioni sull'uso di farmaci.
            </div>
        </div>
    `);
}
...
function showDataSources() {
    const modal = createInfoModal('Fonti di Dati', `
        <div style="line-height: 1.6; color: var(--text-secondary);">
            <h4 style="color: var(--text); margin-bottom: 1rem;">📊 Fonti di Dati</h4>
            
            <h5 style="color: var(--primary); margin: 1.5rem 0 0.5rem;">Database Principale</h5>
            <div style="margin-bottom: 1rem;">
                <strong>Database Farmaci FDA</strong><br>
                <span style="font-size: 0.9rem;">Database ufficiale di farmaci della Food and Drug Administration degli Stati Uniti</span><br>
                <a href="https://api.fda.gov/drug/label.json" target="_blank" rel="noopener" style="color: var(--primary);">→ Visualizza Documentazione API</a>
            </div>
            
            <h5 style="color: var(--primary); margin: 1.5rem 0 0.5rem;">Servizi di Analisi IA</h5>
            <ul style="margin-left: 1rem;">
                <li><strong>OpenAI GPT-4o-mini:</strong> Analisi interazioni farmacologiche</li>
                <li><strong>Anthropic Claude:</strong> Interpretazione informazioni mediche</li>
                <li><strong>Google Gemini:</strong> Analisi multiprospettica</li>
                <li><strong>Perplexity AI:</strong> Ricerca informazioni in tempo reale</li>
            </ul>
            
            <h5 style="color: var(--primary); margin: 1.5rem 0 0.5rem;">Materiali di Riferimento</h5>
            <ul style="margin-left: 1rem;">
                <li><a href="https://www.fda.gov" target="_blank" rel="noopener" style="color: var(--primary);">Sito Web Ufficiale FDA</a></li>
                <li><a href="https://www.drugs.com" target="_blank" rel="noopener" style="color: var(--primary);">Dati Interazioni Drugs.com</a></li>
                <li><a href="https://medlineplus.gov" target="_blank" rel="noopener" style="color: var(--primary);">Informazioni Mediche MedlinePlus</a></li>
            </ul>
            
            <h5 style="color: var(--primary); margin: 1.5rem 0 0.5rem;">Aggiornamenti Dati</h5>
            <p>I dati FDA vengono interrogati in tempo reale e l'analisi IA viene eseguita basandosi sulla conoscenza medica più recente.</p>
            
            <div style="margin-top: 1.5rem; padding: 1rem; background: var(--surface); border-radius: 8px; border: 1px solid var(--border);">
                <strong>ℹ️ Nota:</strong><br>
                Tutti i dati provengono da fonti ufficiali affidabili, ma le situazioni individuali dei pazienti possono variare, quindi la consultazione con professionisti sanitari è essenziale.
            </div>
        </div>
    `);
}
